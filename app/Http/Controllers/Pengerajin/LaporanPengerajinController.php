...
class LaporanPengerajinController extends Controller
{
    ...
    // ======================================================================
    private function getPengerajinContext()
    {
        $userId = Auth::id();
        ...
        // Ambil data pengerajin beserta relasi usahas-nya
        $pengerajin = Pengerajin::with('usahas')->where('user_id', $userId)->first();
        ...
        $usahaIds = $pengerajin->usahas->pluck('id')->toArray();
    ...
    // =========================================================================
    // BASE QUERY UTAMA (untuk index dashboard)
    // ======================================================================
    private function baseQueryPengerajin(Request $request): Builder
    {
        ...
        // --- FILTER WAJIB OTENTIKASI ---
        $base->where('produk.pengerajin_id', $pengerajinId);
        $base->whereIn('usaha_produk.usaha_id', $usahaIds);

        // --- APLIKASI FILTER DARI REQUEST ---
    ...
    public function index(Request $request)
    {
        ...
        $pengerajinList = collect([Auth::user()]); // Untuk View Admin yang di-copy

        // ---------- 2. BASE QUERY OTENTIKASI ----------
        ...
        $totalPendapatan = (clone $baseQuery)->selectRaw('SUM(order_items.quantity * order_items.price_at_purchase) as total')->value('total') ?? 0;
        ...
        $pendapatanPerUsaha = (clone $baseQuery)
            ...
            ->limit(3)
            ->get();
        $pendapatanChart = ['labels' => $pendapatanPerUsaha->pluck('nama_usaha'), 'data' => $pendapatanPerUsaha->pluck('total')];

        // TOP PRODUK
        $topProdukRow = (clone $baseQuery)->selectRaw('produk.nama_produk, SUM(order_items.quantity) as total_qty')->groupBy('produk.id', 'produk.nama_produk')->orderByDesc('total_qty')->first();
        $topProduk = $topProdukRow->nama_produk ?? null;
        $produkTerlaris = (clone $baseQuery)->selectRaw('produk.nama_produk, SUM(order_items.quantity) as total_qty')->groupBy('produk.id', 'produk.nama_produk')->orderByDesc('total_qty')->limit(3)->get();
        $produkTerlarisChart = ['labels' => $produkTerlaris->pluck('nama_produk'), 'data' => $produkTerlaris->pluck('total_qty')];

        // TOP USER (Pembeli Aktif)
        $userAktifRow = (clone $baseQuery)->selectRaw('users.username, COUNT(DISTINCT orders.id) as total_transaksi')->groupBy('users.id', 'users.username')->orderByDesc('total_transaksi')->first();
        $userAktif = $userAktifRow->username ?? null;
        $userAktifList = (clone $baseQuery)->selectRaw('users.username, COUNT(DISTINCT orders.id) as total_transaksi')->groupBy('users.id', 'users.username')->orderByDesc('total_transaksi')->limit(3)->get();
        $transaksiUserChart = ['labels' => $userAktifList->pluck('username'), 'data' => $userAktifList->pluck('total_transaksi')];

        // TOP KATEGORI
        $kategoriTerjual = (clone $baseQuery)->selectRaw('kategori_produk.nama_kategori_produk, SUM(order_items.quantity) as total_qty')->groupBy('kategori_produk.id', 'kategori_produk.nama_kategori_produk')->orderByDesc('total_qty')->limit(3)->get();
        $kategoriChart = ['labels' => $kategoriTerjual->pluck('nama_kategori_produk'), 'data' => $kategoriTerjual->pluck('total_qty')];

        // PRODUK FAVORITE & VIEWS
        $produkBaseQuery = DB::table('produk as p')->where('p.pengerajin_id', $pengerajinId);
        $produkFavorite = (clone $produkBaseQuery)->leftJoin('produk_likes as pl', 'pl.produk_id', '=', 'p.id')->selectRaw('p.nama_produk, COUNT(pl.id) as total_like')->groupBy('p.id', 'p.nama_produk')->orderByDesc('total_like')->limit(3)->get();
        $produkFavoriteChart = ['labels' => $produkFavorite->pluck('nama_produk'), 'data' => $produkFavorite->pluck('total_like')];
        $produkViews = (clone $produkBaseQuery)->leftJoin('produk_views as pv', 'pv.produk_id', '=', 'p.id')->selectRaw('p.nama_produk, COUNT(pv.id) as total_view')->groupBy('p.id', 'p.nama_produk')->orderByDesc('total_view')->limit(3)->get();
        $produkViewChart = ['labels' => $produkViews->pluck('nama_produk'), 'data' => $produkViews->pluck('total_view')];
        ...
        return view('pengerajin.laporan_usaha.laporan', compact(
            ...
            'topProduk',
            'userAktif',
            'pengerajinList' // Dikirim agar view admin tidak error
        ));
    }
    ...
    private function baseKategoriProdukQueryPengerajin(Request $request): Builder
    {
        ...
        // --- FILTER WAJIB OTENTIKASI ---
        $query->where('p.pengerajin_id', $pengerajinId);
    ...
    public function pendapatanUsaha(Request $request)
    {
        ...
        $laporan = $this->basePendapatanUsahaQueryPengerajin($request)
            ...
            ->selectRaw('u.nama_usaha, COUNT(DISTINCT o.id) as total_transaksi, SUM(oi.quantity * oi.price_at_purchase) as total_penjualan, SUM(oi.quantity * oi.price_at_purchase) / NULLIF(COUNT(DISTINCT o.id), 0) as rata_rata_transaksi, MAX(o.created_at) as transaksi_terakhir')
    ...
    public function produkFavorite(Request $request)
    {
        ...
        $totalProduk = Produk::where('pengerajin_id', $pengerajinId)->count();
    ...
    protected function baseProdukSlowMovingQueryPengerajin(Request $request, ?string $start, ?string $end, int $threshold): Builder
    {
        ...
        $query->groupBy('p.id', 'p.nama_produk', 'u.id', 'u.nama_usaha')
            ->selectRaw('u.nama_usaha, p.nama_produk, COALESCE(SUM(oi.quantity), 0) as total_terjual, MAX(oi.created_at) as transaksi_terakhir')
    ...
    protected function baseProdukViewsQueryPengerajin(Request $request): Builder
    {
        ...
        // --- FILTER WAJIB OTENTIKASI ---
        $viewsQuery->where('p.pengerajin_id', $pengerajinId);
    ...
    protected function baseTransaksiQueryPengerajin(Request $request): Builder
    {
        ...
        // --- FILTER WAJIB OTENTIKASI ---
        $base->where('p.pengerajin_id', $pengerajinId);
